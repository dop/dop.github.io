...
The essence of the rule is that all your side-effecting modules (objects) have
to be constructed at the start of your application. This phase could be
understood as configuration part. Once everything is constructed and wired,
execution part begins. In this part only value (pure) objects can be
constructed.
